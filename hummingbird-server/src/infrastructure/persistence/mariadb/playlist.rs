...
#[async_trait]
impl PlaylistDao for MariaDbDatabase {
    async fn list_playlists(&self, user_id: i64) -> Result<Vec<PlaylistWithCount>> {
        let playlists = sqlx::query_as::<_, PlaylistWithCount>(
            "SELECT playlist.id, playlist.name, playlist.created_at, playlist.type, \
             ...
             WHERE playlist.user_id = ? OR playlist.user_id IS NULL \
    ...
    async fn create_playlist(&self, name: &str, user_id: i64) -> Result<i64> {
        let result = sqlx::query("INSERT INTO playlist (name, user_id) VALUES (?, ?)")
            .bind(name)
            .bind(user_id)
            .execute(&self.pool)
            .await?;
    ...
    async fn get_playlist_owner(&self, playlist_id: i64) -> Result<i64> {
        let row: (Option<i64>,) =
            sqlx::query_as("SELECT user_id FROM playlist WHERE id = ?")
                .bind(playlist_id)
                .fetch_one(&self.pool)
                .await?;
